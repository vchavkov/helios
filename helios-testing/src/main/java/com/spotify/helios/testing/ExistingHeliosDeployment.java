...
public class ExistingHeliosDeployment implements HeliosDeployment {
  ...
  private ExistingHeliosDeployment(final Builder builder) {
    ...
    try {
      removeOldJobs(Arrays.asList(firstNonNull(prefixDirectory.toFile().listFiles(), new File[0])),
                    heliosClient);
   ...
   */
  @VisibleForTesting
  static void removeOldJobs(final List<File> files, final HeliosClient heliosClient)
      ...
      try (
        ...
        for (final Map.Entry<JobId, Job> entry : jobs.entrySet()) {
          ...
          final List<AssertionError> errors =
              undeploy(heliosClient, entry.getValue(), hosts, new ArrayList<AssertionError>());
